...
class EdgesOriginCSVWriter:
    ...
    def write_edges(self) -> None:
        ...
        with open(self.output_file, mode="w", newline="") as csvfile:
            ...
            # Write the header
            writer.writerow(
                ["Node ID (sub-cluster)", "Relationship", "Node ID (cluster)"]
            )
        ...
        for sub_cluster in cluster.get_son():
            ...
            for sub_node in sub_nodes:
                for parent_node in parent_nodes:
                    if (
                        sub_node.get_id() != parent_node.get_id()
                        and (sub_node.get_id() + " SUBTYPE " + parent_node.get_id())
                        not in added_rels
                    ):
                        writer.writerow(
                            [sub_node.get_id(), "SUBTYPE", parent_node.get_id()]
                        )
                        added_rels.add(
                            sub_node.get_id() + " SUBTYPE " + parent_node.get_id()
                        )
